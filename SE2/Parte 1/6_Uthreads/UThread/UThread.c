...
//
// The sentinel of the circular list linking the user threads that are alive.
//
static
...
VOID Schedule () {
	...
	if (RunningThread->ToTerminate && RunningThread!=MainThread) UtExit();
}
...
static VOID UtExitHelper() {
	LIST_ENTRY *start = &(RunningThread->JoinSentLink);
	LIST_ENTRY *curr = start->Flink;
	while (curr != start) {
		LINK_BLOCK *Block = CONTAINING_RECORD(curr, LINK_BLOCK, Link);
		PUTHREAD Thread = Block->Thread;
		Thread->NJoin -= 1;
		
		if (Thread->NJoin == 0) UtActivate(Thread);
...
VOID UtExit () {
	...
	if (NextThread != MainThread) NextThread->State = RUNNING;
...
//
HANDLE UtCreate (UT_FUNCTION Function, UT_ARGUMENT Argument, size_t stack_size, const char *name) {
	PUTHREAD Thread;
	...
	Thread->NJoin = 0;
	InitializeListHead(&(Thread->JoinSentLink));
...
VOID UtDump () {
	if (IsListEmpty(&AliveQueue)) {
		printf("No threads alive\n");
	}
	LIST_ENTRY *curr = AliveQueue.Flink;
	PCHAR States[] = { "RUNNING",  "READY",  "BLOCKED" };
	while (curr != &AliveQueue) {
		PUTHREAD thread = CONTAINING_RECORD(curr, UTHREAD, AliveLink);
		UTHREAD_CONTEXT *tc = thread->ThreadContext;
		
		DWORD OccupationStack = 100 - ((((DWORD)tc - (DWORD)thread->Stack) * 100) / ((DWORD)thread->StackSize));
		PCHAR name = thread->Name;
		PCHAR state = States[thread->State];
		printf("%s : %s (Stack Occupied %%= %lu)\n", thread->Name, States[thread->State], OccupationStack);
		curr = curr->Flink;
	}
}

static BOOL CheckDeadLocks(PUTHREAD Thread) {
	LIST_ENTRY *start = &(RunningThread->JoinSentLink);
	LIST_ENTRY *curr = start->Flink;
	while (curr != start) {
		LINK_BLOCK *Block = CONTAINING_RECORD(curr, LINK_BLOCK, Link);
		PUTHREAD bThread = Block->Thread;
		if (Thread == bThread) return TRUE;
	}
	return FALSE;
}

UTHREAD_API BOOL UtMultJoin(HANDLE handle[], int size) {
	for (int i = 0; i < size; i++) {
		PUTHREAD Thread = (PUTHREAD)handle[i];
		
		// Thread cannot join itself
		if (Thread == NULL || Thread == RunningThread) return FALSE;

		// Check for deadlocks
		if (CheckDeadLocks(Thread)) return FALSE;
		
		// Put Running Thread in list of blocked threads of a thread
		LINK_BLOCK *Block = (LINK_BLOCK *) malloc(sizeof(LINK_BLOCK));
		Block->Thread = RunningThread;
		InsertTailList(&(Thread->JoinSentLink), &(Block->Link));
		RunningThread->NJoin += 1;
	}
...
VOID UtTerminate(HANDLE tHandle) {
	PUTHREAD Thread = (PUTHREAD)tHandle;
	if (Thread->State == RUNNING) UtExit();
	else Thread->ToTerminate = TRUE;
}
#else
